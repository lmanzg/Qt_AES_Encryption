...
#define ENCRYPT_KEY "key{0A7910AA-45B7-4E21-818C-43AFAE1029B4}"
#define ENCRYPT_IV  "iv{0A7910AA-45B7-4E21-818C-43AFAE1029B4}"
...
QByteArray QAESEncryption::CBCEncryption(QString str)
{
    QAESEncryption encryption(QAESEncryption::AES_256, QAESEncryption::CBC);
    QString inputStr(str + "@");    //@作为结束符
    QString key(ENCRYPT_KEY);
    QString iv(ENCRYPT_IV);
...
QByteArray QAESEncryption::CBCDecrypt(QByteArray str)
{
    QAESEncryption encryption(QAESEncryption::AES_256, QAESEncryption::CBC);
    ...
    for (int i = 0; i < decodeText.size(); i++)
    {
        if (decodeText[i] == '@')
        {
            decodeText.remove(i, decodeText.size() - i);
        }
    }
